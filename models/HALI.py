...
class DiscriminatorX(nn.Module):
    def __init__(self, zd=128,ch=1,zd1 =64):
        ...
        self.c2  = SpectralNorm(nn.Conv2d(zd1//2, zd1//2, 3,2,1))
...
class DiscriminatorZ1(nn.Module):
    def __init__(self, zd=128,ch=1,zd1=64):
        ...
        self.lr  = nn.LeakyReLU(0.02)
        self.c2  = SpectralNorm(nn.Conv2d(zd1*2, zd//2, 3, 2))
        self.bn2 = nn.BatchNorm2d(zd//2)
        self.c3  = SpectralNorm(nn.Conv2d(zd//2, zd, 3, 1,1))
        self.bn3 = nn.BatchNorm2d(zd)
...
class GeneratorX2(nn.Module):
    def __init__(self, zd=128, ch=1,zd1=64):
        ...
        self.conv1 = nn.ConvTranspose2d(zd1, zd1, 3, 1,1)
        
        self.rn1 =  ResBlock(zd1, zd1)
        
      
        
        self.conv2 = nn.ConvTranspose2d(zd1, zd1, 3, 2,1)
        self.bn2   = nn.BatchNorm2d(zd1)
        
        
        
        self.conv3 =  nn.ConvTranspose2d(zd1, zd1//2, 3,1,1)
        self.bn3 = nn.BatchNorm2d(zd1//2)
        self.conv4 = nn.ConvTranspose2d(zd1//2,ch,2,1)
...
class GeneratorX1(nn.Module):
    def __init__(self, zd=128, ch=1,zd1=64):
        ...
        self.conv2 = nn.ConvTranspose2d(zd, zd, 3, 1)
        self.bn2   = nn.BatchNorm2d(zd)

        self.conv3 =  nn.ConvTranspose2d(zd, zd1*2, 3,1)
        self.bn3   = nn.BatchNorm2d(zd1*2)
      
        self.conv4 =  nn.ConvTranspose2d(zd1*2, zd1*2, 2, 1)
        self.bn4   = nn.BatchNorm2d(zd1*2)
        
        self.conv5 =  nn.ConvTranspose2d(zd1*2, zd1*2, 2, 1)
...
class GeneratorZ1(nn.Module):
    def __init__(self, zd=128, ch=1,zd1 = 64):
      ...
      self.c1 = nn.Conv2d(ch, zd1//4, 3, 1,1)
      
      self.c2  = nn.Conv2d(zd1//4, zd1//2, 3,2,1)
      ...
      self.c3  = nn.Conv2d(zd1//2,zd1,3,2,1) 
      self.bn3 = nn.BatchNorm2d(zd1)
      self.c4 = nn.Conv2d(zd1,zd1*2,3,1,1)
...
class GeneratorZ2(nn.Module):
    def __init__(self, zd=128, zd1=64):
      super().__init__()
      self.c1 = nn.Conv2d(zd1, zd//2, 3, 2)
      self.bn1 = nn.BatchNorm2d(zd//2)
      self.lr  = nn.LeakyReLU(0.02)

      self.c2  = nn.Conv2d(zd//2, zd, 3, 1,1)
      
      self.rn1 =  ResBlock(zd, zd)
      
      self.c3  = nn.Conv2d(zd, zd*2, 3, 1)
